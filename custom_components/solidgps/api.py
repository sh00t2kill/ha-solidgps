...
class SolidGPS:
    ...
    def login(self) -> bool:
        """Log in to SolidGPS and populate account/device info."""
    ...
    def _fetch_nonce(self) -> str:
        _LOGGER.debug("Fetching login page for nonce")
    ...
    def _post_login(self, nonce: str) -> bool:
        _LOGGER.debug("Submitting login for %s", self.username)
        ...
        _LOGGER.debug("Login %s (final URL: %s)", "SUCCESS" if success else "FAILED", resp.url)
        return success

    def _extract_dashboard_info(self):
        _LOGGER.debug("Fetching dashboard")
    ...
    def get_battery_status(self, imei: str | None = None) -> int | None:
        """Return battery percentage (0-100) for a device."""
        if not self.devices:
